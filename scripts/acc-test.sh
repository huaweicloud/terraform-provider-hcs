...
all_files=$(git diff $1 $2 --name-only huaweicloudstack | grep -v "_test.go")
...
for f in $all_files; do
    ...
    if [ "$path" != "huaweicloud" ] && [[ $path != huaweicloudstack/services/* ]]; then
        continue
    fi
...
if [ $coveraged -eq 1 ]; then
    ...
    for f in $all_files; do
        path=${f%/*}
        if [[ $path == huaweicloudstack/services/* ]]; then
