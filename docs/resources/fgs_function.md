...
page_title: "HuaweiCloudStack: hcs_fgs_function"
description: |-
  Manages the function resource within HuaweiCloudStack.
---
...
resource "hcs_fgs_function" "by_swr_image" {
  name        = var.function_name
  agency      = var.agency_name
  handler     = "-"
  app         = "default"
...
resource "hcs_fgs_function" "test" {
  ...
  dns_list   = jsonencode(
    [for v in hcs_dns_zone.test[*] : tomap({id=v.id, domain_name=v.name})]
  )
}
```
...
* `memory_size` - (Required, Int) Specifies the memory size allocated to the function, in MByte (MB). The valid values
  are **128**, **256**, **512**, **768**, **1024**, **1280**, **1536**, **1792**, **2048**, **2560**, **3072**,
  **3584**, **4096**, **8192**, **10240**.

* `runtime` - (Required, String, ForceNew) Specifies the environment for executing the function.  
  The valid values are as follows:
    + **Java8**
    + **Java11**
    + **Java17**. It is only supported in HCS **8.5.0** and **later** version.
    + **Java21**. It is only supported in HCS **8.5.1** and **later** version.
    + **Node.js6.10**
    + **Node.js8.10**
    + **Node.js10.16**
    + **Node.js12.13**
    ...
    + **Node.js16.17**
    + **Node.js18.15**
    + **Node.js20.15**. It is only supported in HCS **8.5.1** and **later** version.
    + **Python2.7**
    + **Python3.6**
    + **Python3.9**
    + **Python3.10**
    + **Python3.12**. It is only supported in HCS **8.5.1** and **later** version.
    + **Go1.x**
    + **C#(.NET Core 2.1)**
    + **C#(.NET Core 3.1)**
    + **C#(.NET Core 6.0)**. It is only supported in HCS **8.5.1** and **later** version.
    + **Custom**
    + **PHP7.3**
    + **PHP8.3**. It is only supported in HCS **8.5.1** and **later** version.
    + **http**
    + **Custom Image**. It is only supported in HCS **8.5.1** and **later** version.
    + **Cangjie1.0**. It is only supported in HCS **8.5.0** and **later** version.
...
* `package` - (Required, String) Specifies the group to which the function belongs.
  This parameter is **Deprecated**, use `app` instead.
...
* `code_type` - (Required, String) Specifies the code type of the function.  
  ...
  -> **Note:** The parameter `custom_image` is **Required** if `code_type` is **Custom-Image-Swr**.
...
* `xrole` - (Optional, String) Specifies the agency configuration of the function. This parameter is **Deprecated**,
  use `agency` instead.
...
* `dns_list` - (Optional, String) Specifies the private DNS configuration of the function network.  
  ...
  `dns_list = jsoncode("[{\"id\":\"ff8080828a07ffea018a17184aa310f5\","domain_name":"functiondebug.example1.com."}]")`
...
* `custom_image` - (Optional, List) Specifies the custom image configuration of the function.  
  ...
  -> **Note:** This parameter is **Required** if `code_type` is **Custom-Image-Swr**.
...
-> **Note:** The `log_group_id`, `log_group_name`, `log_stream_id`, and `log_stream_name` are available and used
together only when `enable_lts_log` is set to **true** or the service default value is **true** (omit `enable_lts_log`).

-> **Note:** When creating a function, if the `enable_lts_log` parameter is set to **true**, and the corresponding
...
* `network_controller` - (Optional, List) Specifies the network configuration of the function.
  ...
  -> **Note:** The network controller can be closed, so **Computed** behavior cannot be supported.
...
* `mount_share_path` - (Optional, String) Specifies the remote mount path, e.g. **192.168.0.12:/data**.

  -> **Note:** This parameter is **Required** if `mount_type` is **ecs**.
...
```bash
$ terraform import hcs_fgs_function.test <id>
```

Note that the imported state may not be identical to your resource definition, due to the attribute missing from the
API response. The missing attributes are: `func_code`, `encrypted_user_data`, `tags`, `network_controller`,
`network_controller.0.disable_public_network`.
...
resource "hcs_fgs_function" "test" {
  ...
  lifecycle {
    ignore_changes = [
      app, func_code, tags, network_controller,
    ]
  }
}
```
