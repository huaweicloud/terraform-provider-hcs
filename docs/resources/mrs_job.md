...
page_title: "HuaweiCloudStack: hcs_mapreduce_job"
description: |-
...
* `program_path` - (Optional, String, ForceNew) Specifies the `.jar` package path or `.py` file path for program
  ...
  ->**Note** This parameter is **Required** if `type` is **MapReduce** or **SparkSubmit**. 

* `parameters` - (Optional, String, ForceNew) Specifies the parameters for the MapReduce job. Add an at sign (@) before
  each parameter can prevent the parameters being saved in plaintext format. Each parameter is separated with spaces.
  This parameter can be set when `type` is **Flink**, **MapReduce** or **SparkSubmit**.
...
* `sql` - (Optional, String, ForceNew) Specifies the SQL command or file path. The parameter is **Required** if `type`
  is **HiveSql** or **SparkSql**.
...
* `status` - The status of the MapReduce job.
