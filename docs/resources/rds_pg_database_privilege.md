...
* `users` - (Required, List) Specifies the account that associated with the database. A single request supports a
  maximum of **50** elements. The [users](#rds_pg_database_privilege_users) structure is documented below.
...
* `schema_name` - (Required, String) Specifies the name of the schema.
  The schema name must be `between 1 and 63` characters long, consisting of letters, numbers, or underscores. It cannot
  contain any other special characters, start with **pg** or a number, or have the same name as the RDS for PostgreSQL
  template library. Additionally, the schema name must already exist.
