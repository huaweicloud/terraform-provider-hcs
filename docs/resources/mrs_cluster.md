...
* `version` - (Required, String, ForceNew) Specifies the MapReduce cluster version. The valid values are as follows:
    + **MRS 3.3.1-LTS**. This value is only supported in `HCS 8.3.1`
    + **MRS 3.5.0-LTS**. This value is only supported in `HCS 8.5.0`
    + **MRS 3.5.1-LTS**. This value is only supported in `HCS 8.5.1`
    + **MRS 3.6.0-LTS**. This value is only supported in `HCS 8.6.0`
...
* `master_nodes` - (Required, List, ForceNew) Specifies the information about master nodes in the MapReduce cluster.
  The [nodes](mrs_cluster_nodes) object structure is documented below.
...
* `type` - (Optional, String, ForceNew) Specifies the type of the MapReduce cluster. Defaults to **ANALYSIS**
...
* `log_collection` - (Optional, Bool, ForceNew) Specifies whether logs are collected when cluster installation fails.
  ...
  ->**Note** If `log_collection` set **true**, the OBS buckets will be created and only used to collect logs that
...
* `node_admin_pass` - (Optional, String, ForceNew) Specifies the administrator password, which is used to log in to the
  ...
  ->**Note** `node_admin_pass` and `node_key_pair` are alternative.
...
* `safe_mode` - (Optional, Bool, ForceNew) Specifies whether the running mode of the MapReduce cluster is secure,
  defaults to **true**. The valid values are as follows:
...
* `template_id` - (Optional, String, ForceNew) Specifies the template used for node deployment when the cluster type is
  **CUSTOM**. Changing this will create a new MapReduce cluster resource.  
  The valid values are as follows:
  ...
  instances are deployed in the same node group. This deployment mode is applicable to a cluster with `100` to `500`
  nodes and delivers better performance in high-concurrency load scenarios.
  + **mgmt_control_data_separated_v2**: The management role and control role are deployed on different Master nodes,
  and data instances are deployed in different node groups. This deployment mode is applicable to a cluster with
  more than 500 nodes. Components can be deployed separately, which can be used for a larger cluster scale.

* `custom_nodes` - (Optional, List, ForceNew) Specifies the information about custom nodes in the MapReduce cluster.  
  The [nodes](#mrs_cluster_nodes) object structure is documented below.
...
* `external_datasources` - (Optional, List, ForceNew) Specifies the external datasource configurations of the cluster.  
  The [external_datasources](#mrs_cluster_external_datasources) structure is documented below.

  Changing this will create a new MapReduce cluster resource.

  ~> **WARNING:** The `external_datasources` will be **Deprecated** in the later version
...
* `bootstrap_scripts` - (Optional, List, ForceNew) Specifies the bootstrap action scripts.
  ...
  The [bootstrap_scripts](#mrs_cluster_bootstrap_scripts) structure is documented below.

  Changing this will create a new MapReduce cluster resource.

<a name="mrs_cluster_nodes"></a>
...
* `group_name` - (Optional, String, ForceNew) Specifies the name of nodes for the node group.
  ...
  ->**Note** This parameter is only valid and **Required** for `custom_nodes`.
...
* `data_volume_count` - (Required, Int, ForceNew) Specifies the data disk number of the nodes. The valid value is range
  from `0` to `10`.

  Changing this will create a new MapReduce cluster resource.

* `data_volume_type` - (Optional, String, ForceNew) Specifies the data disk flavor of the nodes. This parameter is
  **Required** if `data_volume_count` is greater than zero.  
...
* `data_volume_size` - (Optional, Int, ForceNew) Specifies the data disk size of the nodes,in GB. The value range is
  range from `10` to `32768`. The parameter is **Required** if `data_volume_count` is greater than zero.

  Changing this will create a new MapReduce cluster resource.

* `assigned_roles` - (Optional, List, ForceNew) Specifies the roles deployed in a node group. This parameter is
  ...
  **Role expression definition:**
  ...
  ->**Note** `DBService` is a basic component of a cluster. Components such as Hive, Hue, Oozie, Loader, and Redis, and Loader
...
* `configs` - (Required, List, ForceNew) Specifies the configuration of component installed.  
  The [configs](#mrs_cluster_configs) structure is documented below.

  Changing this will create a new MapReduce cluster resource.

<a name="mrs_cluster_configs"></a>
...
<a name="mrs_cluster_external_datasources"></a>
...
<a name="mrs_cluster_bootstrap_scripts"></a>
...
* `fail_action` - (Required, String, ForceNew) Specifies the action after the bootstrap action script fails to be
  executed. Defaults to **errorout**.
  The valid values are as follows:
  + **continue**: Continue to execute subsequent scripts.
  + **errorout**: Stop the action, indicating that the action is stopped.

  Changing this will create a new MapReduce cluster resource.

  ->**Note** You are advised to set this parameter to continue in the commissioning phase so that the cluster can
...
* `node` - The nodes attributes. The [node](mrs_cluster_nodes_attr) object structure is documented below.
    + **master_nodes**
    + **analysis_core_nodes**
...
* `bootstrap_scripts` - (Optional, List, ForceNew) The the bootstrap action scripts.

  The [bootstrap_scripts](#mrs_cluster_bootstrap_scripts_attr) structure is documented below.

<a name="mrs_cluster_nodes_attr"></a>
The `host_ips` block supports:

* `host_ips` - The host list of this nodes group in the cluster.

<a name="mrs_cluster_bootstrap_scripts_attr"></a>
The `bootstrap_scripts` block supports:

* `start_time` - The execution time of one bootstrap action script, in RFC-3339 format.

* `state` - The status of one bootstrap action script.
    + **PENDING**
    + **IN_PROGRESS**
    + **SUCCESS**
    + **FAILURE**
...
`manager_admin_pass`, `node_admin_pass`,`template_id`, `assigned_roles`, `external_datasources`,
