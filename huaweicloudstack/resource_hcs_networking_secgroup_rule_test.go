...
func TestAccNetworkingSecGroupRule_action(t *testing.T) {
	var (
		...
		allowResName string = "hcs_networking_secgroup_rule.allow"
		denyResName  string = "hcs_networking_secgroup_rule.deny"
	)
...
func testAccCheckNetworkingSecGroupRuleDestroy(s *terraform.State) error {
	...
	networkingClient, err := config.NetworkingV1Client(HCS_REGION_NAME)
	if err != nil {
		return fmtp.Errorf("Error creating HuaweiCloudStack networking client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcs_networking_secgroup_rule" {
			continue
		}
...
resource "hcs_networking_secgroup_rule" "allow" {
  security_group_id = hcs_networking_secgroup.secgroup_test.id
  direction         = "ingress"
  ethertype         = "IPv4"
  ports             = 80
...
resource "hcs_networking_secgroup_rule" "deny" {
  security_group_id = hcs_networking_secgroup.secgroup_test.id
  direction         = "ingress"
  ethertype         = "IPv4"
  ports             = 8080
