...
import (
	...
	"github.com/huaweicloud/terraform-provider-hcs/huaweicloudstack/services/cse"
)

func getMicroserviceEngineConfigurationFunc(_ *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
...
func TestAccMicroserviceEngineConfiguration_basic(t *testing.T) {
	var (
		...
		resourceName = "hcs_cse_microservice_engine_configuration.test"
...
func testAccMicroserviceEngineConfiguration_basic_step1(name string) string {
	return fmt.Sprintf(`
data "hcs_cse_microservice_engines" "test" {}

locals {
  id_filter_result = [
    for o in data.hcs_cse_microservice_engines.test.engines : o if o.id == "%[1]s"
  ]
}
...
`, acceptance.HCS_CSE_MICROSERVICE_ENGINE_ID, acceptance.HCS_CSE_MICROSERVICE_ENGINE_ADMIN_PASSWORD, name)
}
