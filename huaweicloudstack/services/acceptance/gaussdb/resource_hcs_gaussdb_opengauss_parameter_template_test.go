...
func getOpenGaussParameterTemplateResourceFunc(cfg *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
...
func TestAccOpenGaussParameterTemplate_basic(t *testing.T) {
	...
	rName := "hcs_gaussdb_opengauss_parameter_template.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "engine_version",
						"hcs_gaussdb_opengauss_parameter_template.source", "engine_version"),
					resource.TestCheckResourceAttrPair(rName, "instance_mode",
						"hcs_gaussdb_opengauss_parameter_template.source", "instance_mode"),
					resource.TestCheckResourceAttrPair(rName, "source_configuration_id",
						"hcs_gaussdb_opengauss_parameter_template.source", "id"),
					resource.TestCheckResourceAttrSet(rName, "created_at"),
					resource.TestCheckResourceAttrSet(rName, "updated_at"),
				),
			},
...
func testOpenGaussParameterTemplate_copy(name string) string {
	return fmt.Sprintf(`
resource "hcs_gaussdb_opengauss_parameter_template" "source" {
  name           = "%[1]s_source"
  description    = "test terraform description"
  engine_version = "8.202"
  instance_mode  = "combined_hcs6"
...
resource "hcs_gaussdb_opengauss_parameter_template" "test" {
  name                    = "%[1]s"
  description             = "test terraform description"
  source_configuration_id = hcs_gaussdb_opengauss_parameter_template.source.id
}
`, name)
}
