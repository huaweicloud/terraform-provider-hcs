...
func getAddressGroupMemberResourceFunc(hcsCfg *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
...
func TestAccAddressGroupMember_basic(t *testing.T) {
	...
	rName := "hcs_cfw_address_group_member.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "group_id", "hcs_cfw_address_group.test", "id"),
...
resource "hcs_cfw_address_group_member" "test" {
  group_id = hcs_cfw_address_group.test.id
  address  = "192.168.0.1"
}
