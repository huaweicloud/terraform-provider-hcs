...
func sfsTurboDirQuotaReadfunc(cfg *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
	var (
...
func TestAccSfsTurboDirQuota_basic(t *testing.T) {
	...
	resourceName := "hcs_sfs_turbo_dir_quota.test"
...
resource "hcs_sfs_turbo_dir_quota" "test" {
  path     = "%[2]s"
  share_id = hcs_sfs_turbo_dir.test.share_id
  capacity = 100
  inode    = 100
}
