...
func sfsTurboDirReadfunc(cfg *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
	var (
...
func TestAccSfsTurboDir_basic(t *testing.T) {
	...
	resourceName := "hcs_sfs_turbo_dir.test"
...
resource "hcs_sfs_turbo_dir" "test" {
  path     = "%[2]s"
  share_id = hcs_sfs_turbo.test.id
  mode     = 777
  gid      = 100
  uid      = 100
}
