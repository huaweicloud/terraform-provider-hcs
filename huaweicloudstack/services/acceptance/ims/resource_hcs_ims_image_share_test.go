...
func TestAccImsImageShare_basic(t *testing.T) {
	...
	rName := "hcs_ims_image_share.test"
...
resource "hcs_ims_image_share" "test" {
 source_image_id    = hcs_ims_image.test.id
 target_project_ids = ["%[2]s"]
}
`, testAccImsImage_basic(imageName), acceptance.HCS_DEST_PROJECT_ID)
}
...
`, testAccImsImage_basic(imageName), acceptance.HCS_DEST_PROJECT_ID_TEST)
}
