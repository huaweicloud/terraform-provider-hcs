...
func TestAccDataSourceStreams_basic(t *testing.T) {
	var (
		dataSource = "data.hcs_lts_streams.test"
		rName      = acceptance.RandomAccResourceName()
		dc         = acceptance.InitDataSourceCheck(dataSource)

		byName   = "data.hcs_lts_streams.filter_by_name"
		dcByName = acceptance.InitDataSourceCheck(byName)

		byLogGroupName   = "data.hcs_lts_streams.filter_by_log_group_name"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(byName, "streams.0.id", "hcs_lts_streams.test", "id"),
...
resource "hcs_lts_streams" "test" {
  group_id    = hcs_lts_group.test.id
  stream_name = hcs_lts_group.test.group_name
  ttl_in_days = 60
...
locals {
  stream_name = hcs_lts_stream.test.stream_name
}

# The name is an exact match.
data "hcs_lts_streams" "filter_by_name" {
  depends_on = [
    hcs_lts_stream.test
  ]
...
locals {
  name_filter_result = [
    for v in data.hcs_lts_streams.filter_by_name.streams[*].name : v == local.stream_name
  ]
}
...
locals {
  log_group_name = hcs_lts_group.test.group_name
}

data "hcs_lts_streams" "filter_by_log_group_name" {
  depends_on = [
    hcs_lts_stream.test
  ]
...
locals {
  stream_ids = data.hcs_lts_streams.filter_by_log_group_name.streams[*].id
}

output "is_log_group_name_filter_useful" {
  value = length(local.stream_ids) == 1 && local.stream_ids[0] == hcs_lts_stream.test.id
}
`, name)
}
...
func testDataSourceStreams_logStreamNotFoundError() string {
	return `
data "hcs_lts_streams" "not_found_log_stream" {
...
func testDataSourceStreams_logGroupNotFoundError() string {
	return `
data "hcs_lts_streams" "not_found_log_group" {
