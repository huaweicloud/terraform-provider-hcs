...
func getMysqlDatabasePrivilegeResourceFunc(cfg *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "db_name",
						"hcs_rds_mysql_database.test", "name"),
					resource.TestCheckResourceAttrPair(rName, "users.0.name",
						"hcs_rds_mysql_account.test_1", "name"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "db_name",
						"hcs_rds_mysql_database.test", "name"),
					resource.TestCheckResourceAttrPair(rName, "users.0.name",
						"hcs_rds_mysql_account.test_2", "name"),
...
resource "hcs_rds_mysql_account" "test_1" {
...
resource "hcs_rds_mysql_database_privilege" "test" {
  ...
  users {
    name = hcs_rds_mysql_account.test_1.name
  }
}
`, testMysqlDatabase_basic(rName, dbPwd, rName), rName, dbPwd)
}
...
resource "hcs_rds_mysql_account" "test_2" {
  instance_id = hcs_rds_instance.test.id
  name        = "%s_2"
...
resource "hcs_rds_mysql_database_privilege" "test" {
  ...
  users {
    name     = hcs_rds_mysql_account.test_2.name
    readonly = true
  }
}
