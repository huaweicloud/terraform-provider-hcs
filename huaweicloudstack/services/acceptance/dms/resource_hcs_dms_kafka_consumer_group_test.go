...
func getDmsKafkaConsumerGroupResourceFunc(cfg *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
...
func TestAccDmsKafkaConsumerGroup_basic(t *testing.T) {
	...
	resourceName := "hcs_dms_kafka_consumer_group.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDmsKafkaConsumerGroup_basic(rName, pwd),
			...
			{
				Config: testDmsKafkaConsumerGroup_basic_update(rName, pwd),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(resourceName, "name", rName),
...
func testDmsKafkaConsumerGroup_basic(name, pwd string) string {
	return fmt.Sprintf(`
%s
...
`, testAccKafkaInstance_basic(name, pwd), name)
}

func testDmsKafkaConsumerGroup_basic_update(name, pwd string) string {
	return fmt.Sprintf(`
%s
