...
func getDmsKafkaPermissionsFunc(c *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
	client, err := c.HcDmsV2Client(acceptance.HCS_REGION_NAME)
	if err != nil {
...
func TestAccDmsKafkaPermissions_basic(t *testing.T) {
	...
	resourceName := "hcs_dms_kafka_permissions.test"
...
resource "hcs_dms_kafka_user" "test1" {
  instance_id = hcs_dms_kafka_instance.test.id
  name        = "%[2]s-1"
  password    = "%[3]s"
}

resource "hcs_dms_kafka_permissions" "test" {
  instance_id = hcs_dms_kafka_instance.test.id
  topic_name  = hcs_dms_kafka_topic.topic.name

  policies {
    user_name     = hcs_dms_kafka_user.test1.name
    access_policy = "all"
  }
}
`, testAccDmsKafkaTopic_basic(rName, password), rName, password)
}
...
resource "hcs_dms_kafka_user" "test2" {
  instance_id = hcs_dms_kafka_instance.test.id
  name        = "%[2]s-2"
  password    = "%[3]s"
}
...
resource "hcs_dms_kafka_permissions" "test" {
  ...
  policies {
    user_name     = hcs_dms_kafka_user.test2.name
    access_policy = "sub"
  }
}
`, testAccDmsKafkaTopic_basic(rName, password), rName, password)
