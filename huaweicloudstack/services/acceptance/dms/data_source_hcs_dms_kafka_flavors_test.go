...
func TestAccKafkaFlavorsDataSource_basic(t *testing.T) {
	dataSourceName := "data.hcs_dms_kafka_flavors.test"
...
data "hcs_dms_kafka_flavors" "baisc" {
  type = "cluster"
}
...
locals {
  test_refer   = data.hcs_dms_kafka_flavors.baisc.flavors[0]
  test_results = data.hcs_dms_kafka_flavors.test
}
