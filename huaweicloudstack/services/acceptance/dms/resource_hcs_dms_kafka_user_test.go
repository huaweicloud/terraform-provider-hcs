...
func getDmsKafkaUserFunc(c *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
...
func TestAccDmsKafkaUser_basic(t *testing.T) {
	...
	resourceName := "hcs_dms_kafka_user.test"
...
resource "hcs_dms_kafka_user" "test" {
  instance_id = hcs_dms_kafka_instance.test.id
  name        = "%[2]s"
  password    = "%[3]s"
}
`, testAccKafkaInstance_basic(rName, password), rName, password)
}
