...
func getRouteTableResourceFunc(conf *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
...
func TestAccVpcRouteTable_basic(t *testing.T) {
	...
	resourceName := "hcs_vpc_route_table.test"
...
resource "hcs_vpc_subnet" "test1-1" {
  name       = "%s-1-1"
  cidr       = "192.168.0.0/24"
...
resource "hcs_vpc_subnet" "test1-2" {
  name       = "%s-1-2"
...
resource "hcs_vpc_subnet" "test2-1" {
  name       = "%s-2-1"
  cidr       = "172.16.10.0/24"
  gateway_ip = "172.16.10.1"
  vpc_id     = hcs_vpc.test2.id
}
...
resource "hcs_vpc_route_table" "test" {
  ...
  subnets     = [
    hcs_vpc_subnet.test1-1.ipv4_subnet_id,
    hcs_vpc_subnet.test1-2.ipv4_subnet_id,
  ]
