...
import (
	...
	"github.com/huaweicloud/terraform-provider-hcs/huaweicloudstack/sdk/huaweicloud/openstack/swr/v2/repositories"

	"github.com/huaweicloud/terraform-provider-hcs/huaweicloudstack/config"
	"github.com/huaweicloud/terraform-provider-hcs/huaweicloudstack/services/acceptance"
)

func getResourceRepository(conf *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
...
func TestAccSWRRepository_basic(t *testing.T) {
	...
	resourceName := "hcs_swr_repository.test"
...
func testAccSWRRepositoryImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		for _, rs := range s.RootModule().Resources {
			if rs.Type == "hcs_swr_organization" {
				organization = rs.Primary.Attributes["name"]
			} else if rs.Type == "hcs_swr_repository" {
...
resource "hcs_swr_repository" "test" {
