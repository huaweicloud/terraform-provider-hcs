...
import (
	...
	"github.com/huaweicloud/terraform-provider-hcs/huaweicloudstack/services/fgs"
)

func getFunction(cfg *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
	client, err := cfg.NewServiceClient("fgs", acceptance.HCS_REGION_NAME)
	if err != nil {
...
func TestAccFunction_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			...
			acceptance.TestAccPreCheckFgsDependencyVersionId(t)
		},
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(withObsStorage, "agency", acceptance.HCS_FGS_AGENCY_NAME),
					...
					resource.TestCheckResourceAttr(withCustomImage, "agency", acceptance.HCS_FGS_AGENCY_NAME),
					resource.TestCheckResourceAttr(withCustomImage, "description", "Created by terraform script"),
					resource.TestCheckResourceAttrPair(withCustomImage, "vpc_id", "hcs_vpc.test", "id"),
					resource.TestCheckResourceAttrPair(withCustomImage, "network_id", "hcs_vpc_subnet.test", "id"),
					...
					resource.TestCheckResourceAttr(withCustomImage, "custom_image.0.url", acceptance.HCS_BUILD_IMAGE_URL),
					...
					resource.TestCheckResourceAttr(withDeprecatedParams, "xrole", acceptance.HCS_FGS_AGENCY_NAME),
				),
			},
			{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(withCustomImage, "app", "default"),
					resource.TestCheckResourceAttr(withCustomImage, "app_agency", acceptance.HCS_FGS_APP_AGENCY_NAME),
					resource.TestCheckResourceAttr(withCustomImage, "handler", "-"),
					...
					resource.TestCheckResourceAttr(withCustomImage, "custom_image.#", "1"),
					resource.TestCheckResourceAttr(withCustomImage, "custom_image.0.url", acceptance.HCS_BUILD_IMAGE_URL_UPDATED),
...
resource "hcs_obs_bucket_object" "test" {
  bucket = hcs_obs_bucket.test.bucket
  key    = "test.zip"
...
resource "hcs_vpc" "trigger_access" {
  count = 3
...
resource "hcs_fgs_function" "with_base64_code" {
  ...
  network_controller {
    ...
    dynamic "trigger_access_vpcs" {
      for_each = slice(hcs_vpc.trigger_access[*].id, 0, 2)
...
resource "hcs_fgs_function" "with_text_code" {
  name                  = "%[2]s-text-code"
  memory_size           = 128
  runtime               = "%[3]s"
  timeout               = 3
...
resource "hcs_fgs_function" "with_obs_storage" {
  ...
  handler               = "index.handler"
  code_type             = "obs"
  code_url              = format("https://%%s/%%s", hcs_obs_bucket.test.bucket_domain_name, hcs_obs_bucket_object.test.key)
  agency                = "%[4]s"
  description           = "Created by terraform script"
  functiongraph_version = "v2"
...
resource "hcs_fgs_function" "with_custom_image" {
...
resource "hcs_fgs_function" "with_deprecated_params" {
...
`, zipFileUploadResourcesConfig(name, runtime), name, runtime,
		acceptance.HCS_FGS_AGENCY_NAME,
		acceptance.HCS_BUILD_IMAGE_URL, acceptance.HCS_FGS_DEPENDENCY_VERSION_ID)
}
...
resource "hcs_fgs_function" "with_base64_code" {
  ...
  network_controller {
    dynamic "trigger_access_vpcs" {
      for_each = slice(hcs_vpc.trigger_access[*].id, 1, 3)
...
`, zipFileUploadResourcesConfig(name, runtime), name, runtime,
		acceptance.HCS_FGS_AGENCY_NAME,
		acceptance.HCS_FGS_APP_AGENCY_NAME,
		acceptance.HCS_BUILD_IMAGE_URL_UPDATED,
		acceptance.HCS_FGS_DEPENDENCY_VERSION_ID)
}
...
func TestAccFunction_withEpsId(t *testing.T) {
	var (
		...
		resourceName = "hcs_fgs_function.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccFunction_withEpsId(name, acceptance.HCS_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
...
resource "hcs_fgs_function" "test" {
  name                  = "%[2]s"
  memory_size           = 128
  runtime               = "Python2.7"
  timeout               = 3
...
func TestAccFunction_logConfig(t *testing.T) {
	var (
		...
		createWithLtsParams      = "hcs_fgs_function.create_with_lts_params"
		rcCreateWithLtsParams    = acceptance.InitResourceCheck(createWithLtsParams, &obj, getFunction)
		createWithoutLtsParams   = "hcs_fgs_function.create_without_lts_params"
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(createWithLtsParams, "functiongraph_version", "v2"),
					resource.TestCheckResourceAttrPair(createWithLtsParams, "log_group_id",
						"hcs_lts_group.test.1", "id"),
					resource.TestCheckResourceAttrPair(createWithLtsParams, "log_group_name",
						"hcs_lts_group.test.1", "group_name"),
					resource.TestCheckResourceAttrPair(createWithLtsParams, "log_stream_id",
						"hcs_lts_stream.test.1", "id"),
					resource.TestCheckResourceAttrPair(createWithLtsParams, "log_stream_name",
						"hcs_lts_stream.test.1", "stream_name"),
					rcCreateWithoutLtsParams.CheckResourceExists(),
					resource.TestCheckResourceAttr(createWithoutLtsParams, "functiongraph_version", "v2"),
...
resource "hcs_lts_stream" "test" {
  ...
  group_id    = hcs_lts_group.test[count.index].id
...
resource "hcs_fgs_function" "create_with_lts_params" {
  ...
  log_group_id    = hcs_lts_group.test[0].id
  log_stream_id   = hcs_lts_stream.test[0].id
  log_group_name  = hcs_lts_group.test[0].group_name
  log_stream_name = hcs_lts_stream.test[0].stream_name
}

resource "hcs_fgs_function" "create_without_lts_params" {
...
`, testAccFunction_logConfig_base(name), name, acceptance.HCS_FGS_AGENCY_NAME)
}
...
resource "hcs_fgs_function" "create_with_lts_params" {
  ...
  log_group_id    = hcs_lts_group.test[1].id
  log_stream_id   = hcs_lts_stream.test[1].id
  log_group_name  = hcs_lts_group.test[1].group_name
  log_stream_name = hcs_lts_stream.test[1].stream_name
}
...
func TestAccFunction_network(t *testing.T) {
	var (
		...
		createWithNetwork   = "hcs_fgs_function.create_with_network"
		rcCreateWithNetwork = acceptance.InitResourceCheck(createWithNetwork, &obj, getFunction)

		createWithoutNetwork   = "hcs_fgs_function.create_without_network"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(createWithNetwork, "agency", acceptance.HCS_FGS_AGENCY_NAME),
					resource.TestCheckResourceAttrPair(createWithNetwork, "vpc_id", "hcs_vpc.test", "id"),
					resource.TestCheckResourceAttrPair(createWithNetwork, "network_id", "hcs_vpc_subnet.test", "id"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet(createWithNetwork, "peering_cidr"),
					rcCreateWithoutNetwork.CheckResourceExists(),
					resource.TestCheckResourceAttr(createWithoutNetwork, "agency", acceptance.HCS_FGS_AGENCY_NAME),
					resource.TestCheckResourceAttrPair(createWithoutNetwork, "vpc_id", "hcs_vpc.test", "id"),
					resource.TestCheckResourceAttrPair(createWithoutNetwork, "network_id", "hcs_vpc_subnet.test", "id"),
...
resource "hcs_vpc_subnet" "test" {
  vpc_id     = hcs_vpc.test.id
  name       = "%[2]s"
  cidr       = cidrsubnet(hcs_vpc.test.cidr, 2, 0)
  gateway_ip = cidrhost(cidrsubnet(hcs_vpc.test.cidr, 2, 0), 1)
}

resource "hcs_vpc" "source" {
  count = 3
...
resource "hcs_vpc_peering_connection" "test" {
  ...
  vpc_id      = hcs_vpc.source[count.index].id
  peer_vpc_id = hcs_vpc.target[count.index].id
}
...
resource "hcs_fgs_function" "create_with_network" {
  ...
  vpc_id       = hcs_vpc.test.id
  network_id   = hcs_vpc_subnet.test.id
  peering_cidr = join(";", slice(hcs_vpc.target[*].cidr, 0, 2))
  dns_list     = jsonencode(
    [for v in slice(hcs_dns_zone.test[*], 0, 2) : tomap({id=v.id, domain_name=v.name})]
  )
}

resource "hcs_fgs_function" "create_without_network" {
...
`, testAccFunction_network_base(name), name, acceptance.HCS_FGS_AGENCY_NAME)
}
...
resource "hcs_fgs_function" "create_with_network" {
  ...
  vpc_id       = hcs_vpc.test.id
  network_id   = hcs_vpc_subnet.test.id
  peering_cidr = join(";", slice(hcs_vpc.target[*].cidr, 1, 3))
  dns_list     = jsonencode(
    [for v in slice(hcs_dns_zone.test[*], 1, 3) : tomap({id=v.id, domain_name=v.name})]
  )
}
...
func TestAccFunction_reservedInstance(t *testing.T) {
	var (
		...
		withVersion   = "hcs_fgs_function.with_version"
		rcWithVersion = acceptance.InitResourceCheck(withVersion, &obj, getFunction)

		withAlias   = "hcs_fgs_function.with_alias"
...
resource "hcs_fgs_function" "with_version" {
...
resource "hcs_fgs_function" "with_alias" {
  name        = "%[2]s_with_alias"
  memory_size = 128
  runtime     = "Node.js16.17"
  timeout     = 3
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "gpu_type", acceptance.HCS_FGS_GPU_TYPE),
				),
			},
			{
...
`, functionScriptVariableDefinition, name, memory, acceptance.HCS_FGS_GPU_TYPE)
}
...
`, name, acceptance.HCS_FGS_AGENCY_NAME)
}
