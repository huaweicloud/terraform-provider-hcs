...
func getDcsAccountResourceFunc(cfg *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
...
func TestAccDcsAccount_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDcsAccount_basic(accountName, pwd),
			...
			{
				Config: testAccDcsAccount_updated(accountName, pwd),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(rName, "account_name", accountName),
...
func testAccDcsAccount_basic(accountName, pwd string) string {
	return fmt.Sprintf(`
%s
...
}`, testAccDcsV1Instance_basic(accountName, pwd), accountName)
}

func testAccDcsAccount_updated(accountName, pwd string) string {
	return fmt.Sprintf(`
%s
