...
func getAlertRuleResourceFunc(cfg *config.HcsConfig, state *terraform.ResourceState) (interface{}, error) {
...
func TestAccAlertRule_basic(t *testing.T) {
	...
	rName := "hcs_secmaster_alert_rule.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "pipeline_id", acceptance.HCS_SECMASTER_PIPELINE_ID),
...
func testAlertRule_basic(name string) string {
	return fmt.Sprintf(`
resource "hcs_secmaster_alert_rule" "test" {
  workspace_id = "%s"
  pipeline_id  = "%s"
  name         = "%s"
...
`, acceptance.HCS_SECMASTER_WORKSPACE_ID, acceptance.HCS_SECMASTER_PIPELINE_ID, name)
}
