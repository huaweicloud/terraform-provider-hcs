...
func getRulePreciseProtectionResourceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	preciseProtectionClient, err := cfg.NewServiceClient(product, acceptance.HCS_REGION_NAME)
	if err != nil {
...
func TestAccRulePreciseProtection_basic(t *testing.T) {
	...
	rName := "hcs_waf_rule_precise_protection.test"
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttrPair(rName, "policy_id", "hcs_waf_policy.policy_1", "id"),
					resource.TestCheckResourceAttrPair(rName, "conditions.1.reference_table_id", "hcs_waf_reference_table.ref_table", "id"),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "policy_id",
						"hcs_waf_policy.policy_1", "id"),
					resource.TestCheckResourceAttr(rName, "name", name),
					resource.TestCheckResourceAttr(rName, "priority", "20"),
					resource.TestCheckResourceAttr(rName, "action", "block"),
					resource.TestCheckResourceAttrPair(rName, "known_attack_source_id",
						"hcs_waf_rule_known_attack_source.test", "id"),
					resource.TestCheckResourceAttr(rName, "description", "description information"),
				),
			},
			{
...
func TestAccRulePreciseProtection_WithEpsID(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testRulePreciseProtection_basicWithEpsID(name, acceptance.HCS_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
			...
			{
				Config: testRulePreciseProtection_updateWithEpsID(name, acceptance.HCS_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
					resource.TestCheckResourceAttr(rName, "enterprise_project_id", acceptance.HCS_ENTERPRISE_PROJECT_ID_TEST),
					resource.TestCheckResourceAttr(rName, "name", fmt.Sprintf("%s_update", name)),
					...
					resource.TestCheckResourceAttrPair(rName, "conditions.0.reference_table_id", "hcs_waf_reference_table.ref_table", "id"),
				),
			},
			{
				ResourceName:      rName,
...
resource "hcs_waf_rule_precise_protection" "test" {
...
resource "hcs_waf_rule_precise_protection" "test" {
  ...
  conditions {
    ...
    logic              = "prefix_any"
    subfield           = "test_sub"
    reference_table_id = hcs_waf_reference_table.ref_table.id
  }
}
...
resource "hcs_waf_rule_precise_protection" "test" {
  ...
  priority               = 20
  action                 = "block"
  known_attack_source_id = hcs_waf_rule_known_attack_source.test.id
  description            = "description information"
  ...
  depends_on = [
    hcs_waf_dedicated_domain.domain_1
  ]
}
