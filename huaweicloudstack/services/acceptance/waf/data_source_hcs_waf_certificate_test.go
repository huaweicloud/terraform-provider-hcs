...
func TestAccDataSourceWafCertificateV1_basic(t *testing.T) {
	...
	dataSourceName := "data.hcs_waf_certificate.cert_1"
...
func TestAccDataSourceWafCertificateV1_withEpsID(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccWafCertificateListV1_conf_withEpsID(name, acceptance.HCS_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckWafCertDataSourceID(dataSourceName),
...
data "hcs_waf_certificate" "cert_1" {
  name          = hcs_waf_certificate.certificate_1.name
  expire_status = 1
