...
func TestAccDataSourceWafPoliciesV1_basic(t *testing.T) {
	...
	dataSourceName := "data.hcs_waf_policies.policies_1"
...
func TestAccDataSourceWafPoliciesV1_withEpsID(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccWafPoliciesV1_conf_epsID(name, acceptance.HCS_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckWafPoliciesID(dataSourceName),
...
data "hcs_waf_policies" "policies_1" {
  name = hcs_waf_policy.policy_1.name
}
