...
func TestAccObsBucket_withEpsId(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccObsBucket_epsId(rInt, acceptance.HCS_ENTERPRISE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckObsBucketExists(resourceName),
			...
			{
				Config: testAccObsBucket_epsId(rInt, acceptance.HCS_ENTERPRISE_MIGRATE_PROJECT_ID_TEST),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckObsBucketExists(resourceName),
					resource.TestCheckResourceAttr(
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "user_domain_names.0", "www.example1.com"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "bucket", testAccObsBucketName(rInt)),
					resource.TestCheckResourceAttr(resourceName, "user_domain_names.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "user_domain_names.0", "www.example2.com"),
				),
			},
			{
...
func testAccObsBucketDomainName(randInt int) string {
	return fmt.Sprintf("tf-test-bucket-%d.obs.%s.myhuaweicloud.com", randInt, acceptance.HCS_REGION_NAME)
}
...
resource "hcs_obs_bucket" "bucket" {
  ...
  acl           = "private"
  encryption    = true
  kms_key_id    = hcs_kms_key.key_1.id
...
resource "hcs_obs_bucket" "bucket" {
  ...
  user_domain_names = ["www.example1.com"]
}
`, randInt)
}
...
resource "hcs_obs_bucket" "bucket" {
  ...
  user_domain_names = ["www.example2.com"]
}
`, randInt)
}
