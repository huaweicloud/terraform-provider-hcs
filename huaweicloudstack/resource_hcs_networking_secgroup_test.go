		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV3SecGroupExists(resourceName, &secGroup),
					resource.TestCheckResourceAttr(resourceName, "name", name),
					resource.TestCheckResourceAttr(resourceName, "enterprise_project_id", HCS_ENTERPRISE_PROJECT_ID_TEST),
				),
			},
		},
	})
...
func testAccCheckNetworkingV3SecGroupDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmtp.Errorf("Error creating HuaweiCloudStack networking v3 client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "hcs_networking_secgroup" {
			continue
		}
...
`, name, HCS_ENTERPRISE_PROJECT_ID_TEST)
}
